...
- **Local ML Models vs. Free-Tier Deployment Constraints**:
  - *Trade-off*: Running `sentence-transformers` and PyTorch natively on the backend server introduces severe **Memory Limitations** that prevent us from bringing the application live using standard free hosting environments.
  - *Why*: The machine learning model and PyTorch use a significant chunk of memory (~300MB - 400MB) just to run idle. Free tiers on platforms (like Render's Free tier which offers 512MB RAM) frequently fail during the build or startup process with "Out of Memory" errors. **Because of this heavy technological footprint, we are currently unable to deploy the live website.** To actually host this prototype online, it would require paying for dedicated machine resources (e.g., Render's $7/month tier).
